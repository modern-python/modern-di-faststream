...
`modern-di-faststream` is a FastStream adapter over
...
All implementation is `modern_di_faststream/main.py`, short enough to read whole. Read it.

### Testing patterns

The suite runs against a real broker double, not mocks: `TestNatsBroker` + `TestApp` from
FastStream, with `tests/dependencies.py` as the `Group` every test resolves from. A test that needs
DI wired takes the `app` fixture from `tests/conftest.py`.
...
| Home | Holds |
|---|---|
| `modern_di_faststream/` | anything readable from the module — the default |
...
> Can an agent get this by reading `modern_di_faststream/`? → **don't write it.**
